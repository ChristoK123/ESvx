...
Meteor.startup(function () {
  ...
  if(scoreInterval>0){
    ...
    intervalId=Meteor.setInterval(function () {
      ...
      //$ne checks for all posts in the Posts collection that are NOT inactive
      //the rest updates the score of each post.
      Posts.find({'inactive': {$ne : true}}).forEach(function (post) {
        updatedPosts += updateScore({collection: Posts, item: post});
      });
      //same as above, but for comments instead of posts.
