//creates collection called Posts.
...
//all logged in users may create a post. 
Posts.allow({
    insert: canPostById
  , update: canEditById
  , remove: canEditById
...
//users have a max of 30 posts per 24 hour period.
Meteor.methods({
  post: function(post){
    ...
    //retrieves all relevant data for a post.
    post = _.extend(post, {
      headline: headline,
      body: body,
      userId: userId,
  ...
  //increases the number of clicks on a click event of a post.
  ...
  //deletes everything related to the post.
