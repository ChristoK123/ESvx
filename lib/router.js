...
//NOTE: Admin is being given the error message, but still able to delete a post.
//MYTODO: Fix this.
 ...
 //checks to see if the user has filled in his/her role in profile.
   noRoleProfile: function() { //add an if for if no one is logged in.
    this.subscribe('currentUser').wait();
    if(profile.role !="Educator")  {
      this.render('roleSubmit');
    } else {
      this.stop();
    }
    },
...
if(Meteor.isClient){
  ...
  Router.before(filters.noRoleProfile, {except: ['loading', 'user_profile', 'user_edit']});
